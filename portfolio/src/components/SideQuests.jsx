    ...
    <Section id="quests" eyebrow="off the résumé" title="Side quests">
      <ol className="divide-y divide-line border-y border-line">
        {sideQuests.map(({ name, role, detail, tags }, i) => (
          ...
          >
            <p aria-hidden="true" className="font-mono text-xs text-red">
              {String(i + 1).padStart(2, '0')}
            </p>
            <div>
              <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                <h3 className="font-display text-2xl text-cream">{name}</h3>
                <p className="font-hand text-base text-red">{role}</p>
              </div>
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-tan">{detail}</p>
              <p className="mt-2 font-mono text-xs uppercase tracking-[0.15em] text-tan/60">
                {tags}
              </p>
            </div>
          </motion.li>
